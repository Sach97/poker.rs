...
mod tests {
    ...
    fn compare_two_hands2() {
        ...
        let player_rank = player.rank();
        assert_eq!(player_rank, Rank::StraightFlush(Face::Six));
        let opponent = hand::Hand::from_vec(vec!["As", "Ad", "Ac", "Ah", "Jd"]);
        let opponent_rank = opponent.rank();
        assert_eq!(opponent_rank, Rank::FourOfAKind(Face::Ace));
        assert_eq!(player > opponent, true);
    }

    #[test]
    fn compare_two_hands3() {
        let player = hand::Hand::from_vec(vec!["As", "Ah", "2h", "Ad", "Ac"]);
        let player_rank = player.rank();
        assert_eq!(player_rank, Rank::FourOfAKind(Face::Ace));
        let opponent = hand::Hand::from_vec(vec!["Js", "Jd", "Jc", "Jh", "3d"]);
        let opponent_rank = opponent.rank();
    ...
    fn compare_two_hands4() {
        let player = hand::Hand::from_vec(vec!["As", "3s", "4s", "8s", "2s"]);
        let player_rank = player.rank();
        assert_eq!(player_rank, Rank::Flush(Face::Ace));
        let opponent = hand::Hand::from_vec(vec!["2h", "3h", "5h", "6h", "7h"]);
        let opponent_rank = opponent.rank();
    ...
    fn compare_two_hands5() {
        ...
        let player_rank = player.rank();
        assert_eq!(player_rank, Rank::StraightFlush(Face::Six));
        let opponent = hand::Hand::from_vec(vec!["4h", "5h", "6h", "7h", "8h"]);
        let opponent_rank = opponent.rank();
    ...
    fn compare_two_hands6() {
        let player = hand::Hand::from_vec(vec!["6s", "Ad", "7h", "4s", "As"]);
        let player_rank = player.rank();
        assert_eq!(player_rank, Rank::Pair(Face::Ace, Face::Seven));
        let opponent = hand::Hand::from_vec(vec!["Ah", "Ac", "5h", "6h", "9s"]);
        let opponent_rank = opponent.rank();
    ...
    fn compare_two_hands7() {
        let player = hand::Hand::from_vec(vec!["2s", "3h", "6h", "7s", "9c"]);
        let player_rank = player.rank();
        assert_eq!(player_rank, Rank::HighCard(Face::Nine));
        let opponent = hand::Hand::from_vec(vec!["7h", "3c", "10h", "6h", "9s"]);
        let opponent_rank = opponent.rank();
